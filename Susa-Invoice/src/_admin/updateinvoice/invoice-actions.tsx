...
export default function InvoiceActions({
  ...
  // Alternative: Use html2canvas with better settings
  ...
  const createPDFFromImage = async (imageData: string, copyType: string) => {
    ...
    return new Promise<void>((resolve, reject) => {
      img.onload = () => {
        ...
        pdf.save(`Invoice_${copyType}.pdf`)
        resolve()
      }
      img.onerror = () => {
  ...
  const handleGeneratePdfAndSave = async (invoiceType: "Proforma" | "Tax") => {
    setIsProgressInvoice(invoiceType === "Proforma")
    ...
    try {
      ...
      alert(`Invoice updated successfully as ${invoiceType} Invoice!`)

      // Then, generate PDFs
      setIsEditingMode(false) // Switch to preview mode for capture
      setShowOriginalLabel(true)
      setShowDuplicateLabel(false)
      await new Promise((resolve) => setTimeout(resolve, 500)) // Give a moment for UI to update
      const elementScreenshot = await captureElementScreenshot()
      await createPDFFromImage(elementScreenshot, "ORIGINAL")

      setShowOriginalLabel(false)
      setShowDuplicateLabel(true)
      await new Promise((resolve) => setTimeout(resolve, 500)) // Give a moment for UI to update
      const elementScreenshot2 = await captureElementScreenshot()
      await createPDFFromImage(elementScreenshot2, "DUPLICATE")
      ...
      setShowOriginalLabel(false)
      setShowDuplicateLabel(false)
      setIsEditingMode(true) // Revert to edit mode
    }
  }
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", gap: "16px", marginTop: "24px" }}>
      ...
      <button
        onClick={() => handleGeneratePdfAndSave("Proforma")}
        disabled={isGeneratingPDF || isSaving || isEditingMode} // Disabled in edit mode
        style={{
          display: "flex",
          alignItems: "center",
          gap: "8px",
          ...
          cursor: "pointer",
          transition: "background-color 0.3s",
          opacity: isGeneratingPDF || isSaving || isEditingMode ? 0.5 : 1,
        }}
      >
        <FileText style={{ width: "16px", height: "16px" }} />
        {isGeneratingPDF && isProgressInvoice ? "Generating Proforma..." : "Proforma PDF"}
      </button>
      <button
        onClick={() => handleGeneratePdfAndSave("Tax")}
        disabled={isGeneratingPDF || isSaving || isEditingMode} // Disabled in edit mode
        style={{
          display: "flex",
          alignItems: "center",
      ...
      >
        <FileText style={{ width: "16px", height: "16px" }} />
        {isGeneratingPDF && !isProgressInvoice ? "Generating Tax..." : "Tax PDF"}
      </button>
    </div>
  )
}
